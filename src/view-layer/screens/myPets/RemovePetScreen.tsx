...
import { useRemoveAnimalMutation } from "./hooks/useRemoveAnimalMutation";
...
export function RemovePetScreen({ route, navigation }: StackProps) {
  ...
  const removeAnimalMutation = useRemoveAnimalMutation();
  const onRemoveAnimal = useCallback(() => {
    if (!animal) return;
    removeAnimalMutation.mutate({
      animalId: id,
    });
